    ...
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((b) => (
            <Card key={b.name} className="glass p-5 flex flex-col hover:border-primary/50 transition-all">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {b.cashless && <Badge className="bg-accent/10 text-accent border-accent/20 text-[10px]">⚡ Cashless</Badge>}
                </div>
              </div>
              <ul className="flex flex-wrap gap-x-3 gap-y-1 mb-3 text-[11px] text-muted-foreground">
                {b.highlights.map((h) => (
              ...
              <div className="space-y-2 mb-3 flex-1">
                <p className="text-xs font-semibold text-primary uppercase tracking-wide">Popular Plans</p>
                {b.plans.map((p) => (
                  <a key={p.planName} href={p.link} target="_blank" rel="noopener noreferrer"
                     className="block p-3 rounded-xl border border-border/40 bg-muted/20 hover:bg-primary/5 hover:border-primary/40 transition">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h4 className="font-semibold text-sm">{p.planName}</h4>
                      <span className="text-xs font-bold text-accent whitespace-nowrap">{p.startingPrice}</span>
                    </div>
                    <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                      <span>💰 {p.coverage}</span>
                      <ExternalLink className="h-3 w-3" />
                    </div>
                    <p className="text-[11px] text-muted-foreground mt-1">✓ {p.bestFor}</p>
                  </a>
                ))}
              </div>
              ...
              <div className="flex gap-2 mt-auto">
                ...
                {b.helpline && (
                  <a href={`tel:${b.helpline}`}>
                    <Button size="sm" variant="outline" className="glass text-xs"><Phone className="h-3 w-3 mr-1" />{b.helpline}</Button>
                  </a>
                )}
              </div>
            </Card>
